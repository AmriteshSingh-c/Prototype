...
if (isset($_POST)) {
	...
	$sql = "SELECT id_coordinator, name, email, active FROM coordinator WHERE email='$email' AND password='$password' ";
	...
	if ($result->num_rows > 0) {
		...
		while ($row = $result->fetch_assoc()) {
			...
			if ($row['active'] == '2') {
				$_SESSION['coordinatorLoginError'] = "Your Account Is Still Pending Approval.";
				header("Location: coordinator-login.php");
				exit();
			} else if ($row['active'] == '0') {
				$_SESSION['coordinatorLoginError'] = "Your Account Is Rejected. Please Contact For More Info.";
				header("Location: coordinator-login.php");
				exit();
				...
				$_SESSION['coordinatorLoginError'] = "Your Account Is Deactivated. Contact Admin For Reactivation.";
				header("Location: coordinator-login.php");
				exit();
			}
		}
