...
if (isset($_POST)) {
	...
	$sql1 = "SELECT email FROM coordinator WHERE email='$email'";
	...
	if ($result->num_rows != 0) {
		...
		if ($uploadOk == false) {
			header("Location: coordinator-register.php");
			exit();
		}

		//sql new registration insert query
		$sql = "INSERT INTO coordinator(name, email,id_coordinator, password) VALUES ('$name', '$email',(SELECT id_user from users where email='$email'), '$password')";
